...
class Database:
    ...
    async def init(self):
        async with self.connect() as conn:
            ...
            """)
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_questions_to_user ON questions(to_user_id)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_questions_reply ON questions(reply_to_id)")
            await conn.commit()

    # --- Пользователи ---
    async def get_or_create_user(self, user):
        async with self.connect() as conn:
            row = await conn.execute("SELECT user_id, link FROM users WHERE user_id = ?", (user.id,))
            ...
            if res:
                ...
                await conn.commit()
                return res["user_id"], res["link"]
            else:
                link = self._generate_link()
                while True:
                    chk = await conn.execute("SELECT 1 FROM users WHERE link = ?", (link,))
