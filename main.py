...
# ----- Обработка ответов (исправлено с диагностикой) -----
...
async def reply_text(message: types.Message, state: FSMContext):
    ...
    if not q_id or not to_user:
        await message.answer(
            "❌ Ошибка: данные утеряны. Попробуй снова нажать 'Ответить' на вопрос.",
    ...
    try:
        ...
        error_text = f"❌ Ошибка при отправке ответа: {type(e).__name__}: {str(e)}"
        logger.error(error_text)
        await message.answer(error_text, reply_markup=main_kb(message.from_user.id))
    finally:
        await state.clear()
...
async def ask_button(message: types.Message):
    await message.answer(
        "✏️ Чтобы задать вопрос, перейди по ссылке пользователя. Если уже перешёл — просто напиши вопрос.",
        reply_markup=cancel_kb()
    )
...
async def incoming_button(message: types.Message):
    ...
    await message.answer("📥 Входящие вопросы:", reply_markup=inline_incoming(qs, 0, has_more))
...
async def mylink_button(message: types.Message):
    ...
    if row:
        bot_info = await bot.get_me()
        await message.answer(
            f"🔗 Твоя ссылка:\n<code>https://t.me/{bot_info.username}?start={row[0]}</code>",
            reply_markup=main_kb(message.from_user.id),
            parse_mode="HTML"
        )
    else:
...
async def back_incoming(callback: types.CallbackQuery):
    ...
    if not qs:
        ...
        return
    has_more = len(get_incoming(user_id, 1, 10)) > 0
    await callback.message.answer("📥 Входящие вопросы:", reply_markup=inline_incoming(qs, 0, has_more))
    await callback.answer()
...
async def main():
    init_db()  # гарантия создания таблиц при старте
